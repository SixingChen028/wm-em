...
class IndexSerialRecallEnv(ImmediateSerialRecallEnv):
    """
    An index immediate serial recall environment.
    """
    ...
    def __init__(
        ...
        Construct an environment.
        """
        super().__init__(num_items, num_targets, t_delay, seed)
    ...
    def step(self, action):
        ...
        # decision stage
            ...
            coordinate = np.array([0., 0.])
            if action == self.t_elapsed - self.num_items - self.t_delay: # action should be the index
                reward += 1
